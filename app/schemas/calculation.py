...
class CalculationBase(BaseModel):
    ...
    def validate_inputs(self) -> "CalculationBase":
        ...
        if self.type == CalculationType.POWER:
            # Power operation must have exactly two inputs: base and exponent
            if len(self.inputs) != 2:
...
class CalculationUpdate(BaseModel):
    ...
    # Allow updating inputs and/or the calculation type
    ...
    def validate_inputs(self) -> "CalculationUpdate":
        ...
        if self.inputs is not None and len(self.inputs) < 2:
            raise ValueError("At least two numbers are required for calculation")
        # If type is POWER and inputs provided, ensure exactly two numbers
        if self.type == CalculationType.POWER and self.inputs is not None:
            if len(self.inputs) != 2:
                raise ValueError("Power operation requires exactly two numbers: base and exponent")
        return self
